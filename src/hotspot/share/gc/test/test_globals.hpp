...
#ifndef SHARE_GC_TEST_TEST_GLOBALS_HPP
#define SHARE_GC_TEST_TEST_GLOBALS_HPP

#include "runtime/globals_shared.hpp"
//
// Defines all globals flags used by the TEST GC.
//

#define GC_TEST_FLAGS(develop,                                           \
                         develop_pd,                                        \
                         product,                                           \
                         product_pd,                                        \
                         notproduct,                                        \
                         ...
                         constraint)                                        \
                                                                            \
  product(size_t, TestPrintHeapSteps, 20, EXPERIMENTAL,                  \
          "Print heap occupancy stats with this number of steps. "          \
          "0 turns the printing off.")                                      \
          range(0, max_intx)                                                \
                                                                            \
  product(size_t, TestUpdateCountersStep, 1 * M, EXPERIMENTAL,           \
          ...
          range(1, max_intx)                                                \
                                                                            \
  product(size_t, TestMaxTLABSize, 4 * M, EXPERIMENTAL,                  \
          "Max TLAB size to use with Test GC. Larger value improves "    \
          "performance at the expense of per-thread memory waste. This "    \
          "asks TLAB machinery to cap TLAB sizes at this value.")           \
          range(1, max_intx)                                                \
                                                                            \
  product(bool, TestElasticTLAB, true, EXPERIMENTAL,                     \
          ...
          "smaller TLABs until policy catches up.")                         \
                                                                            \
  product(bool, TestElasticTLABDecay, true, EXPERIMENTAL,                \
          ...
          "large burst.")                                                   \
                                                                            \
  product(double, TestTLABElasticity, 1.1, EXPERIMENTAL,                 \
          ...
          range(1.0, DBL_MAX)                                               \
                                                                            \
  product(size_t, TestTLABDecayTime, 1000, EXPERIMENTAL,                 \
          ...
          range(1, max_intx)                                                \
                                                                            \
  product(size_t, TestMinHeapExpand, 128 * M, EXPERIMENTAL,              \
          "Min expansion step for heap. Larger value improves performance " \
          "at the potential expense of memory waste.")                      \
          range(1, max_intx)

// end of GC_TEST_FLAGS

#endif // SHARE_GC_TEST_TEST_GLOBALS_HPP
