...
#include "gc/test/testInitLogger.hpp"
#include "gc/test/testMemoryPool.hpp"
#include "gc/test/testThreadLocalData.hpp"
...
jint TestHeap::initialize() {
  ...
  _decay_time_ns = (int64_t) EpsilonTLABDecayTime * NANOSECS_PER_MILLISEC;
    ...
    // Enable monitoring
    _monitoring_support = new TestMonitoringSupport(this);
    _last_counter_update = 0;
    _last_heap_print = 0;

  // Install barrier set
  BarrierSet::set_barrier_set(new TestBarrierSet());

    // All done, print out the configuration
    TestInitLogger::print();

    return JNI_OK;
}

void TestHeap::initialize_serviceability() {
    _pool = new TestMemoryPool(this);
    _memory_manager.add_pool(_pool);
}

GrowableArray<GCMemoryManager *> TestHeap::memory_managers() {
    GrowableArray<GCMemoryManager *> memory_managers(1);
    memory_managers.append(&_memory_manager);
    return memory_managers;
}

GrowableArray<MemoryPool *> TestHeap::memory_pools() {
    GrowableArray<MemoryPool *> memory_pools(1);
    memory_pools.append(_pool);
    return memory_pools;
}

size_t TestHeap::unsafe_max_tlab_alloc(Thread *thr) const {
...
TestHeap *TestHeap::heap() {
    return named_heap<TestHeap>(TestHeap::Test);
}

HeapWord *TestHeap::allocate_work(size_t size, bool verbose) {
...
HeapWord *TestHeap::allocate_new_tlab(size_t min_size,
    ...
    if (EpsilonElasticTLAB) {
        ergo_tlab = TestThreadLocalData::ergo_tlab_size(thread);

        if (EpsilonElasticTLABDecay) {
            int64_t last_time = TestThreadLocalData::last_tlab_time(thread);
            ...
            if (last_time != 0 && (time - last_time > _decay_time_ns)) {
                ergo_tlab = 0;
                TestThreadLocalData::set_ergo_tlab_size(thread, 0);
            }
        }
  ...
  assert(size <= CollectedHeap::max_tlab_size(),
    ...
    if (res != nullptr) {
        ...
        *actual_size = size;
        if (EpsilonElasticTLABDecay) {
            TestThreadLocalData::set_last_tlab_time(thread, time);
        }
        if (EpsilonElasticTLAB && !fits) {
            // If we requested expansion, this is our new ergonomic TLAB size
            TestThreadLocalData::set_ergo_tlab_size(thread, size);
        }
    } else {
...
HeapWord *TestHeap::mem_allocate(size_t size, bool *gc_overhead_limit_was_exceeded) {
    *gc_overhead_limit_was_exceeded = false;
    return allocate_work(size);
}

HeapWord *TestHeap::allocate_loaded_archive_space(size_t size) {
    // Cannot use verbose=true because Metaspace is not initialized
    return allocate_work(size, /* verbose = */false);
}

void TestHeap::collect(GCCause::Cause cause) {
    switch (cause) {
        case GCCause::_metadata_GC_threshold:
        case GCCause::_metadata_GC_clear_soft_refs:
            // Receiving these causes means the VM itself entered the safepoint for metadata collection.
            // While Test does not do GC, it has to perform sizing adjustments, otherwise we would
...
void TestHeap::do_full_collection(bool clear_all_soft_refs) {
    collect(gc_cause());
}

void TestHeap::object_iterate(ObjectClosure *cl) {
    _space->object_iterate(cl);
}

void TestHeap::print_on(outputStream *st) const {
...
bool TestHeap::print_location(outputStream* st, void* addr) const {
  return BlockLocationPrinter<TestHeap>::print_location(st, addr);
}

void TestHeap::print_tracing_info() const {
  print_heap_info(used());
  print_metaspace_info();
}

void TestHeap::print_heap_info(size_t used) const {
...
void TestHeap::print_metaspace_info() const {
