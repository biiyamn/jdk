...
#include "gc/test/testMonitoringSupport.hpp"
...
class TestSpaceCounters: public CHeapObj<mtGC> {
...
public:
  TestSpaceCounters(const char* name,
                 int ordinal,
                 size_t max_size,
...
class TestGenerationCounters : public GenerationCounters {
private:
  TestHeap* _heap;
public:
  TestGenerationCounters(TestHeap* heap) :
...
TestMonitoringSupport::TestMonitoringSupport(TestHeap* heap) {
  _heap_counters  = new TestGenerationCounters(heap);
  _space_counters = new TestSpaceCounters("Heap", 0, heap->max_capacity(), 0, _heap_counters);
}

void TestMonitoringSupport::update_counters() {
  MemoryService::track_memory_usage();

  if (UsePerfData) {
    TestHeap* heap = TestHeap::heap();
